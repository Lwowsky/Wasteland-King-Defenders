...
(function () {
      ...
      a
        ...
        .forEach((e) => {
          r.push({
            ...
            helpers: Math.max(
              0,
              Number(e.querySelector("[data-calc-helpers]")?.value || 15) || 15,
            ),
          });
        }),
      r
  ...
  function L(e) {
    ...
    (a.length && (t.shift1 = a),
      ...
      (t.minHelpersPerTower = !!e?.querySelector("#towerCalcMinHelpersOn")
        ?.checked),
      (t.minHelpersCount = Math.max(
        1,
        Math.min(
          30,
          Number(
            e?.querySelector("#towerCalcMinHelpersCount")?.value ||
              t.minHelpersCount ||
              10,
          ) || 10,
        ),
    ...
    try {
      localStorage.setItem("pns_tower_calc_state", JSON.stringify(t));
    } catch {}
    return t;
  }
  function c() {
    ...
    return (
      ...
      (e.minHelpersPerTower =
        "boolean" == typeof e.minHelpersPerTower && e.minHelpersPerTower),
      (e.minHelpersCount = Math.max(
        1,
        Math.min(30, Number(e.minHelpersCount || 10) || 10),
      )),
  ...
  function de(t, n, o, s = new Set()) {
    ...
    for (let e = 0; e < C.length; e++) {
      ...
      if (l > 0 && o?.minHelpersPerTower) {
        const e = Math.max(
          1,
          Math.min(30, Number(o?.minHelpersCount || 10) || 10),
        );
        l = Math.max(l, e);
      }
    ...
    const t = L(e),
      n = {
        ...
        minHelpersCount: Math.max(
          1,
          Math.min(30, Number(t.minHelpersCount || 10) || 10),
        ),
