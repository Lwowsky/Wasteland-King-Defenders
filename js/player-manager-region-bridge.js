import { makePublicShareUrl, rememberShareCode } from './core/share-links.js?v=86';
...
import { canDeleteRegionRegistration, canEditRegionTowerPlan, canManageRegion, deleteRegionAlliance as deleteRegionAllianceDb, deleteRegionRegistrations, getManagedRegionOptions, getRegionTowerPlan, shareRegionFinalPlan as shareRegionFinalPlanDb, listRegionAlliances as listRegionAlliancesDb, listRegionCatalog, listRegionRegistrations, regionRegistrationToPlayer, saveRegionAlliance as saveRegionAllianceDb, saveRegionTowerPlan, updateRegionRegistration } from './services/region-db.js?v=86';
...
async function refreshTowerRegionOptions() {
  ...
  if (currentUser && currentProfile && isProfileComplete(currentProfile)) {
    ...
    if (isOwnerAdmin() || role === 'admin' || role === 'moderator') {
      const catalog = await listRegionCatalog({ includeInactive: true }).catch(error => {
