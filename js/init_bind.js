(function () {
  ...
  function getDefaultAutoFillSettings() {
    ...
    return {
      ...
      maxHelpers: PNS.clampInt(
        c.maxHelpers?.defaultValue ?? c.maxHelpers?.value ?? 29,
      ),
    };
  }
  ...
  function saveAutoFillSettings(settings) {
    if (typeof PNS.safeWriteJSON === "function")
      PNS.safeWriteJSON(PNS.KEYS.KEY_AUTOFILL_SETTINGS, settings);
  }
  ...
  function writeAutoFillSettingsToUI(settings) {
    ...
    if (c.maxHelpers)
      c.maxHelpers.value = PNS.clampInt(settings?.maxHelpers, 29);
  }
  ...
  function initFromDomOnce() {
    ...
    if (typeof PNS.parsePlayersFromTable === "function")
      PNS.parsePlayersFromTable();
    if (typeof PNS.parseBasesFromCards === "function")
      PNS.parseBasesFromCards();

    // actions UI
    if (typeof PNS.buildRowActions === "function") PNS.buildRowActions();

    // initial render (safe)
    if (!state._didInitialRender && typeof PNS.renderAll === "function") {
      PNS.renderAll();
  ...
  function reattachPlayersToDom() {
    ...
    const table =
      PNS.controls?.playersDataTable ||
      document.querySelector("#playersDataTable");
    if (!table) return;
  ...
  function rehydrateAfterDomRefresh() {
    ...
    else if (typeof PNS.parsePlayersFromTable === "function")
      PNS.parsePlayersFromTable();

    if ((state.bases || []).length) reattachBasesToDom();
    else if (typeof PNS.parseBasesFromCards === "function")
      PNS.parseBasesFromCards();

    // re-bind filters (they use dataset guards)
    if (typeof PNS.bindTopFilterEvents === "function")
  ...
  function bindDelegatedEventsOnce() {
    ...
    onClick("#openSettingsBtn", (e) => {
      e.preventDefault();
      PNS.openModal?.("settings");
    });
    onClick("#openBoardBtn", (e) => {
      e.preventDefault();
      PNS.openModal?.("board");
    });
    onClick("#autoFillAllHeaderBtn", (e) => {
      e.preventDefault();
      PNS.autoFillAllVisibleBases?.();
    });

    // drawer buttons (mobile)
    onClick("#openSettingsBtnMobile", (e) => {
      e.preventDefault();
      PNS.openModal?.("settings");
    });
    onClick("#openBoardBtnMobile", (e) => {
      e.preventDefault();
      PNS.openModal?.("board");
    });
    onClick("#autoFillAllHeaderBtnMobile", (e) => {
      e.preventDefault();
      PNS.autoFillAllVisibleBases?.();
    });

    // players header panel actions
    onClick("#showAllDataBtn", (e) => {
      e.preventDefault();
      PNS.toggleColumns?.();
    });
    onClick("#showAllColumnsBtn", (e) => {
      e.preventDefault();
      PNS.toggleColumns?.();
    });

    // bases panel
    onClick("#autoFillAllBasesBtn", (e) => {
      e.preventDefault();
      PNS.autoFillAllVisibleBases?.();
    });

    // board modal
    onClick("#exportPngBtn", (e) => {
      e.preventDefault();
      exportBoardAsPNG();
    });
    onClick("#exportPdfBtn", (e) => {
      e.preventDefault();
      exportBoardAsPDF();
    });

    // import wizard
    onClick("#loadUrlMockBtn", (e) => {
      e.preventDefault();
      PNS.handleLoadUrlClick?.();
    });
    onClick("#useTemplateMockBtn", (e) => {
      e.preventDefault();
      PNS.handleUseSavedTemplateClick?.();
    });
    onClick("#saveVisibleColumnsMockBtn", (e) => {
      e.preventDefault();
      PNS.handleSaveVisibleColumnsClick?.();
    });
    onClick("#loadDemoImportBtn", (e) => {
      e.preventDefault();
      PNS.loadDemoIntoImportWizard?.();
    });
    onClick("#saveTemplateMockBtn", (e) => {
      e.preventDefault();
      PNS.saveCurrentImportTemplate?.();
    });
    onClick("#applyImportMockBtn", (e) => {
      e.preventDefault();
      PNS.applyImportedPlayers?.();
    });
    ...
    document.addEventListener("click", (e) => {
      if (typeof PNS.handleShiftTabClick === "function")
        PNS.handleShiftTabClick(e);
    });
    ...
    document.addEventListener("change", (e) => {
      ...
      if (typeof PNS.setBaseTowerRule === "function") {
        PNS.setBaseTowerRule(baseId, PNS.readBaseEditorSettingsInputs?.(base), {
          persist: true,
          rerender: true,
        });
      }
  ...
  function init() {
    ...
    if (typeof PNS.loadBaseTowerRulesStore === "function")
    ...
    if (typeof PNS.applyColumnVisibility === "function") {
      const saved =
        typeof PNS.safeReadBool === "function"
    ...
    if (typeof PNS.applyShiftFilter === "function") {
      const savedShift = (() => {
        try {
          return (
            localStorage.getItem(PNS.KEYS.KEY_SHIFT_FILTER) ||
            state.activeShift ||
            "shift2"
          );
        } catch {
          return state.activeShift || "shift2";
        }
      })();
    ...
    if (typeof PNS.bindTopFilterEvents === "function")
      PNS.bindTopFilterEvents();
    if (typeof PNS.applyPlayerTableFilters === "function")
