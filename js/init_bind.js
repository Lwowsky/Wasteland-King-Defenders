(function () {
  ...
  function reattachBasesToDom() {
    ...
    boardCols.forEach((col) => {
      const h4 = (col.querySelector('h4')?.textContent || '').trim();
      const k = slug(String(h4).split('/')[0].trim());
      if (k && !boardMap.has(k)) boardMap.set(k, col);
    });

    cards.forEach((card, idx) => {
      const titleText = (card.querySelector('h3')?.textContent || '').trim() || `Base ${idx + 1}`;
      const baseKey = slug(titleText.split('/')[0]);
      const id = `b-${idx + 1}-${baseKey}`;
  ...
  function bindDelegatedEventsOnce() {
    ...
    onClick('#openImportQuickBtn', (e) => { e.preventDefault(); PNS.openModal?.('settings'); });
    ...
    document.addEventListener('keydown', (e) => {
      ...
      if (!(state.activeModal || location.hash === '#settings-modal' || location.hash === '#board-modal')) return;
      PNS.closeModal?.();
      if (location.hash === '#settings-modal' || location.hash === '#board-modal') {
        try { history.replaceState(null, '', location.pathname + location.search); } catch { location.hash = ''; }
      }
