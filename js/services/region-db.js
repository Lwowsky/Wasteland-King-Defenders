...
import { readCache, writeCache, removeCache } from './local-cache.js?v=079';
import { trackReads, trackWrites, trackDeletes } from './usage-tracker.js?v=079';
import {
  getUserProfile,
  getFarmById,
  getGameProfile,
...
async function getRegionSettingsFirestore(region) {
  const { db, firestoreMod } = await getFirebaseParts();
  const safeRegion = normalizeRegion(region);
  if (!safeRegion) throw new Error('region-required');
  const ref = firestoreMod.doc(db, 'regions', safeRegion);
...
export async function getRegionSettings(region) {
  ...
  return getRegionSettingsFirestore(safeRegion);
}

async function getRegionSettingsD1First(region) {
  return getRegionSettings(region);
}
...
async function actionLogCacheModule() {
  return import('./action-log-cache.js?v=079');
}
...
async function writeRegionActionLog(firebase, user, profile = {}, region = '', action = '', details = {}) {
  try {
    ...
    const payload = {
      ...
      actorAccess: actorAccessForRegionPayload(user, profile || {}, safeRegion)
    };
    try {
...
export async function listRegionActionLogs(user, regionOverride = '', { limitCount = 20, cursorMs = 0 } = {}) {
  ...
  try {
    const mod = await actionLogCacheModule();
    if (mod?.isActionLogCacheEnabled?.()) {
      const result = await mod.listRegionActionLogsD1(user, region, { limitCount: limitValue, cursorMs: safeCursorMs, alliance: role === 'officer' ? ownAlliance : '', actorAccess: actorAccessForRegionPayload(user, profile || {}, region) });
...
async function rotateRegionPublicSharesForNewCycle(user, region, actorAccess = null) {
  try {
    const mod = await import('./region-table-cache.js?v=079');
...
async function readFinalPlanFromD1Cache(code, options = {}) {
  try {
    const mod = await import('./final-plan-cache.js?v=079');
...
function normalizeExtraSquads(values = {}, settings = {}) {
  ...
  const mainTroop = trim(values.troopType);
  ...
  return source
    ...
    .filter(item => item.troopType && item.troopType !== mainTroop && item.tier && !seen.has(item.troopType) && seen.add(item.troopType))
...
function validateRegistration(data = {}, settings = {}) {
  ...
  if (!data.region || !data.nickname || !data.alliance || !data.shift || !data.troopType || !data.tier || !data.lairLevel || !data.marchSize || !data.rallySize) {
  ...
  if (data.extraEnabled) {
    ...
    data.extraSquads.forEach(item => {
      ...
      if (item.troopType === data.troopType) throw new Error('registration-invalid-extra-troop');
