...
function withAdminTimeout(promise, ms = 3500, fallback = null) {
  let timer = null;
  return Promise.race([
    Promise.resolve(promise).catch(error => {
      console.warn('[WKD] admin request failed:', error?.code || error?.message || error);
      return fallback;
    }),
    new Promise(resolve => {
...
async function fetchAdminPublicCacheJson(file, { force = false } = {}) {
  let lastError = null;
...
function adminPublicPlayerToUser(row = {}) {
  const nickname = normalizeText(row.nickname || row.gameNick || row.name || '');
  const region = normalizeText(row.region || '').replace(/[^0-9]/g, '');
  const alliance = allianceTag3(row.alliance || '');
  const publicKey = normalizeText(row.publicKey || row.id || '');
  const publicUid = normalizeText(row.uid || '');
  const uid = publicUid || (publicKey ? `public:${publicKey}` : `public:${region}:${alliance}:${nickname.toLowerCase()}`);
  const gameProfile = {
    nickname,
    region,
    alliance,
    rank: normalizeText(row.rank || 'p1').toLowerCase(),
    shk: normalizeText(row.shk || ''),
    role: normalizeText(row.role || 'player').toLowerCase()
  };
  return {
    id: uid,
    uid,
    publicKey,
    __publicCacheOnly: !publicUid,
    __publicCacheBuild: ADMIN_PUBLIC_CACHE_BUILD,
    email: normalizeText(row.email || row.authEmail || ''),
    ...
    roleLabel: roleLabel(gameProfile.role),
...
function adminPublicFarmToFarm(row = {}) {
  return {
    farmId: normalizeText(row.farmKey || row.farmId || row.id || ''),
    id: normalizeText(row.farmKey || row.farmId || row.id || ''),
    ownerPublicKey: normalizeText(row.ownerPublicKey || row.publicKey || ''),
    ownerUid: normalizeText(row.ownerUid || row.uid || ''),
    ownerNickname: normalizeText(row.ownerNickname || row.mainNickname || ''),
    nickname: normalizeText(row.nickname || row.gameNick || row.name || ''),
    region: normalizeText(row.region || '').replace(/[^0-9]/g, ''),
    alliance: allianceTag3(row.alliance || ''),
    rank: normalizeText(row.rank || 'p1').toLowerCase(),
    shk: normalizeText(row.shk || ''),
    role: normalizeText(row.role || 'player').toLowerCase(),
    createdAt: row.createdAt || row.updatedAt || null,
    updatedAt: row.updatedAt || row.createdAt || null,
    __publicCacheFarm: true
  };
}

function attachAdminPublicFarmsToUsers(usersFromCache = [], farmsFromCache = []) {
  const farms = (Array.isArray(farmsFromCache) ? farmsFromCache : [])
    .map(adminPublicFarmToFarm)
    .filter(farm => farm.nickname || farm.region || farm.alliance);
  if (!farms.length) return usersFromCache.map(user => ({ ...user, farms: Array.isArray(user.farms) ? user.farms : [] }));

  const byOwnerKey = new Map();
  const add = (key, farm) => {
    const clean = normalizeText(key);
    if (!clean) return;
    if (!byOwnerKey.has(clean)) byOwnerKey.set(clean, []);
    byOwnerKey.get(clean).push(farm);
  };

  farms.forEach(farm => {
    add(`publicKey:${farm.ownerPublicKey}`, farm);
    add(`uid:${farm.ownerUid}`, farm);
    add(`nickname:${farm.ownerNickname.toLowerCase()}`, farm);
  });

  return usersFromCache.map(user => {
    const publicKey = normalizeText(user.publicKey || '');
    const uid = normalizeText(user.uid || user.id || '');
    const nickname = normalizeText(user.nickname || user.gameNick || '').toLowerCase();
    const matched = [
      ...byOwnerKey.get(`publicKey:${publicKey}`) || [],
      ...byOwnerKey.get(`uid:${uid}`) || [],
      ...byOwnerKey.get(`nickname:${nickname}`) || []
    ];
    const seen = new Set();
    const mergedFarms = [...(Array.isArray(user.farms) ? user.farms : []), ...matched].filter(farm => {
      const key = normalizeText(farm.farmId || farm.id || farm.nickname || `${farm.region}:${farm.alliance}`);
    ...
    return {
      ...user,
      farmCount: Math.max(Number(user.farmCount || 0), mergedFarms.length),
      farms: mergedFarms
    };
  });
}

async function loadAdminPublicCacheUsers({ force = false } = {}) {
...
function adminPlayerIdentity(user = {}) {
  ...
  const region = normalizeText(game.region || user.region || '').replace(/[^0-9]/g, '');
  const alliance = allianceTag3(game.alliance || user.alliance || '').toLowerCase();
  const nickname = normalizeText(game.nickname || user.nickname || user.gameNick || '').toLowerCase();
  if (region && alliance && nickname) return `game:${region}|${alliance}|${nickname}`;
  const uid = normalizeText(user.uid || user.id || '');
...
function userRow(row) {
  ...
  return `<tr ${rowAttrs} class="${row.isFarmRow ? 'is-farm-row' : 'is-main-row'} ${publicOnly ? 'is-public-cache-row' : ''}">
    ...
    <td class="admin-row-actions" data-label="${labels.actions}">${actionsHtml}</td>
  </tr>`;
}
...
function adminPlayersBodyIsLoading() {
  const body = $('#registeredPlayersBody');
  return Boolean(body && /Завантажую гравців|Loading Players|loading players/i.test(body.textContent || ''));
}

function adminSnapshotRowMatches(row, filters = filterValues()) {
  const game = row?.game || {};
  if (filters.role !== 'all' && rowRole(row) !== filters.role) return false;
  const nick = String(game.nickname || '').toLowerCase();
  const mainNick = String(row.mainNickname || '').toLowerCase();
  const alliance = allianceTag3(game.alliance || '');
  const filterAlliance = allianceTag3(filters.alliance || '');
  const region = String(game.region || '').toLowerCase();
  if (filters.nick && !nick.includes(filters.nick) && !mainNick.includes(filters.nick)) return false;
  if (filterAlliance && alliance !== filterAlliance) return false;
  if (filters.region && !region.includes(filters.region.replace(/[^0-9]/g, ''))) return false;
  return true;
}

function renderUsersDirectFromSnapshot(snapshotUsers = []) {
  const body = $('#registeredPlayersBody');
  if (!body) return 0;
  const rows = (Array.isArray(snapshotUsers) ? snapshotUsers : [])
    .flatMap(user => {
      const main = mainRowForUser(user);
      return includeAdminFarmRows() ? [main, ...farmRowsForUser(user, main)] : [main];
    })
    .filter(row => adminSnapshotRowMatches(row));
  if (!rows.length) {
    body.innerHTML = `<tr><td colspan="8">${escapeHtml(t('admin.noPlayers', 'Гравців не знайдено.'))}</td></tr>`;
    return 0;
  }
...
async function paintAdminPublicCacheFallback({ force = false, reason = 'fallback' } = {}) {
  const body = $('#registeredPlayersBody');
  if (!body) return [];
  const snapshotUsers = await withAdminTimeout(loadAdminPublicCacheUsers({ force }), 1600, []) || [];
  if (!snapshotUsers.length) return [];
  users = mergeAdminUsersWithPublicCache([currentAdminSelfUser()].filter(Boolean), snapshotUsers);
  playersPageMeta = { hasNext: false, reads: 0, queryMode: `public-cache-${reason}-${snapshotUsers.length}`, firstDoc: null, lastDoc: null };
  let shown = 0;
  try {
    renderStats();
    renderUsers();
    shown = adminPlayersBodyIsLoading() ? 0 : adminRows().filter(row => rowMatchesFilters(row)).length;
  } catch (error) {
    console.warn('[WKD] admin normal render failed, direct snapshot render used:', error?.message || error);
  }
  if (!shown || adminPlayersBodyIsLoading()) shown = renderUsersDirectFromSnapshot(users);
  setSummary(tv('admin.playersLoadedSummary', '{count} гравців завантажено • {shown} рядків показано зараз • — заявок', { count: users.length, shown }));
  setStatus(tv('admin.playersLoadedPublicCache', 'Завантажено {count} гравців із public-cache. Редагуються: {editable}. Public-only: {publicOnly}. Firebase reads≈{reads}.', {
    count: users.length,
    editable: users.filter(user => !Boolean(user.__publicCacheOnly || String(user.uid || '').startsWith('public:'))).length,
    publicOnly: users.filter(user => Boolean(user.__publicCacheOnly || String(user.uid || '').startsWith('public:'))).length,
    reads: 0
  }), 'success');
...
async function loadPlayersPage({ reset = false, direction = 'next' } = {}) {
  ...
  let fallbackTimer = setTimeout(() => {
    if (adminPlayersBodyIsLoading()) paintAdminPublicCacheFallback({ force: false, reason: 'timer' }).catch(error => {
      console.warn('[WKD] admin fallback timer failed:', error?.message || error);
    });
  }, 900);

  let publicCacheUsers = [];
  try {
    publicCacheUsers = await withAdminTimeout(loadAdminPublicCacheUsers({ force: reset }), 1800, []) || [];
  } catch (error) {
  ...
  // Editable UID resolving is secondary and non-blocking for the visible list.
  withAdminTimeout(listPublicPlayersForAdmin({
    pageSize: 2000,
    filters: adminPlayerQueryFilters(),
    scanLimit: 2000
  }), 4500, { users: [], reads: 0, queryMode: 'publicPlayers-timeout' })
    .then(publicResult => {
    ...
    .catch(error => console.warn('[WKD] admin editable resolve skipped:', error?.code || error?.message || error));
}
...
function switchTab(tab) {
  ...
  document.querySelectorAll('[data-limit-panel]').forEach(panel => { panel.hidden = safeTab !== 'limits' || !panel.classList.contains('is-active'); });
...
function switchLimitTab(tab = 'firebase') {
  ...
  document.querySelectorAll('[data-limit-panel]').forEach(panel => { const active = panel.dataset.limitPanel === safeTab; panel.classList.toggle('is-active', active); panel.hidden = !active; });
  renderUsage();
}
...
async function initAdminPage() {
  ...
  setTimeout(() => {
    if (adminPlayersBodyIsLoading()) paintAdminPublicCacheFallback({ force: false, reason: 'preauth' }).catch(console.warn);
  }, 1200);
