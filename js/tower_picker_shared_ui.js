(function () {
  ...
  UI.renderTowerPickerListItem = function renderTowerPickerListItem(model) {
    ...
    return `<button type="button" class="btn btn-sm tower-picker-item ${model?.active ? 'active ' : ''}${done ? 'is-ready tower-done' : 'is-not-ready'}"${renderDataAttrs(attrs)}><div class="tower-item-row"><strong>${esc(model?.title || '')}</strong><span class="tower-item-status" aria-hidden="true">${statusIcon}</span></div><span class="muted small">${statusLabel} · <span class="tower-item-count ${countCls}">players: ${playersCount}</span></span></button>`;
  };
  ...
  UI.renderTowerPickerDetail = function renderTowerPickerDetail(model) {
    ...
    const tierMinMarch = model?.rule?.tierMinMarch || {};
    return `
      <div class="stack tower-picker-scope"${wrapperAttrs}>
        ...
        <div class="picker-topline top-space"><select id="towerPickerCaptainSelect" class="input-like" aria-label="Вибір капітана"><option value="">${captain ? 'Змінити капітана…' : 'Вибрати капітана…'}</option>${captainOptions}</select><div class="picker-actions"><button class="btn btn-sm" type="button" data-picker-set-captain="${esc(model?.baseId || '')}">Поставити капітана</button><button class="btn btn-sm" type="button" data-picker-autofill="${esc(model?.baseId || '')}">Автозаповнення</button><button class="btn btn-sm" type="button" data-picker-clear-base="${esc(model?.baseId || '')}">Очистити башню</button><button class="btn btn-sm" type="button" data-picker-save-board="${esc(model?.baseId || '')}">Зберегти таблицю башні</button></div></div>
        <div class="limit-grid limit-grid-compact top-space"><div><span>Captain march</span><strong>${fmt(model?.stats?.captainMarch)}</strong></div><div><span>${rallyLabel}</span><strong>${fmt(model?.stats?.rallySize)}</strong></div><div><span>Total Σ</span><strong>${fmt(model?.stats?.total)}</strong></div><div><span>Free space</span><strong>${fmt(model?.stats?.free)}</strong></div></div>
        <details class="tower-collapsible top-space" id="towerPickerLimitsBlock"><summary>Налаштування башні · ліміти по тірам (макс. March)</summary><div class="inner stack"><div class="picker-limits-head"><label><span class="muted small">Макс. гравців</span><input id="pickerMaxHelpers" type="number" min="0" value="${maxHelpers}" /></label><button class="btn btn-sm" type="button" data-picker-save-rule="${esc(model?.baseId || '')}">Зберегти ліміти</button><button class="btn btn-sm" type="button" data-picker-recalc-rule="${esc(model?.baseId || '')}">Перерахувати склад</button><button class="btn btn-sm" type="button" data-picker-reset-rule="${esc(model?.baseId || '')}">Скинути ліміти (T14–T9 → 0)</button></div><div class="row gap wrap">${['T14','T13','T12','T11','T10','T9'].map(t => `<label><span class="muted small">${t}</span><input type="number" min="0" data-picker-tier="${t}" value="${Number(tierMinMarch?.[t] || 0) || 0}" style="width:90px" /></label>`).join('')}</div><div class="muted small">0 = гнучкий tier: бере повний March, але якщо місця не вистачає — ділить залишок між гравцями цього tier.</div></div></details>
        <details class="tower-collapsible" id="towerPickerManualBlock"><summary>${manualSummary}</summary><div class="inner stack"><div class="picker-manual-row"><input id="pickerManualSearch" list="pickerManualPlayerSuggestions" placeholder="Пошук гравця (зі списку)" autocomplete="off" spellcheck="false" /><input id="pickerManualName" placeholder="Нік (можна свій, не зі списку)" autocomplete="off" /><datalist id="pickerManualPlayerSuggestions">${suggestions}</datalist><input id="pickerManualAlly" placeholder="Альянс" /><select id="pickerManualRole"><option>Fighter</option><option>Shooter</option><option>Rider</option></select></div><div id="pickerManualHint" class="picker-manual-hint muted small"></div><div class="picker-manual-row2"><input id="pickerManualTier" placeholder="T14" /><input id="pickerManualMarch" placeholder="March" type="number" min="0" /><input id="pickerManualRally" placeholder="${rallyPlaceholder}" type="number" min="0" /><button class="btn btn-sm" type="button" data-picker-add-manual-captain="${esc(model?.baseId || '')}">${addManualCaptainLabel}</button><button class="btn btn-sm" type="button" data-picker-add-manual="${esc(model?.baseId || '')}">${addManualLabel}</button></div></div></details>
        <div class="panel subpanel" style="padding:10px"><div class="row gap wrap" style="justify-content:space-between"><strong>Гравці в башні</strong><span class="muted small">${rightText}</span></div><div class="helpers-table-wrap top-space"><table class="mini-table"><thead><tr><th>Player</th><th>Ally</th><th>Role</th><th>Tier</th><th>March</th><th>✎</th></tr></thead><tbody>${rowsHtml}</tbody></table></div></div>
      </div>`;
  };
})();
