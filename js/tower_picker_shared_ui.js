  ...
  UI.renderTowerPickerDetail = function renderTowerPickerDetail(model) {
    ...
    return `
      <div class="stack tower-picker-scope"${wrapperAttrs}>
        ...
        <details class="tower-collapsible top-space" id="towerPickerLimitsBlock"><summary>Налаштування башні · ліміти по тірам (макс. March)</summary><div class="inner stack"><div class="picker-limits-head"><label><span class="muted small">Макс. гравців</span><input id="pickerMaxHelpers" type="number" min="0" value="${maxHelpers}" /></label><button class="btn btn-sm" type="button" data-picker-save-rule="${esc(model?.baseId || '')}">Зберегти ліміти</button><button class="btn btn-sm" type="button" data-picker-reset-rule="${esc(model?.baseId || '')}">Скинути ліміти (T14–T9 → 0)</button></div><div class="row gap wrap">${['T14','T13','T12','T11','T10','T9'].map(t => `<label><span class="muted small">${t}</span><input type="number" min="0" data-picker-tier="${t}" value="${Number(tierMinMarch?.[t] || 0) || 0}" style="width:90px" /></label>`).join('')}</div></div></details>
