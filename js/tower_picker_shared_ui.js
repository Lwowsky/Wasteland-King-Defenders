(function () {
  ...
  function renderCaptainOptions(options, selectedId) {
    return (options || []).map((p) => {
      ...
      const label = `${String(p?.name || '')} · ${typeof window.PNS?.roleLabel === 'function' ? window.PNS.roleLabel(String(p?.role || ''), false) : String(p?.role || '')} · ${typeof window.PNS?.shiftLabel === 'function' ? window.PNS.shiftLabel(shiftLabel) : shiftLabel} · ${fmt(p?.march)}${p?.captainReady ? ' · КАП' : ''}`;
  ...
  UI.renderTowerPickerDetail = function renderTowerPickerDetail(model) {
    ...
    return `
      <div class="stack tower-picker-scope"${wrapperAttrs}>
        ...
        <div class="picker-meta-row muted small"><span class="picker-meta-shift">Зміна: ${esc(model?.shiftLabel || '')}</span>${checkboxes}</div>
